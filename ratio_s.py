...
# ⭐ 셔터 소리 재생용 JavaScript
# 페이지에 숨겨진 <audio> 태그를 찾아서 재생 명령만 내립니다.
JS_SHUTTER_PLAY = """
<script>
    var audio = document.getElementById('shutter_sound_preloaded');
    if (audio) {
        ...
        if (playPromise !== undefined) {
            playPromise.then(function() {
                ...
                // 재생 실패 (Autoplay Policy 등)
                console.error("Audio Playback Failed (Autoplay Policy/Source Error):", error);
            });
        }
    }
</script>
...
def main():
    ...
    # ⭐ 1. 페이지에 오디오 태그를 숨겨서 미리 로드합니다.
    # 주의: shutter.wav 파일은 이 파이썬 파일과 동일한 디렉토리에 있어야 합니다.
    st.markdown("""
    <audio id="shutter_sound_preloaded" src="shutter.wav" preload="auto" style="display:none;"></audio>
    ...
    if webrtc_ctx.state.playing and webrtc_ctx.video_processor:
        ...
        while True:
            ...
            # ⭐ 셔터 소리 신호 확인 및 JavaScript 실행 (소리 재생)
            try:
                if processor.shutter_queue.get(timeout=0.1):
                    # 신호가 오면 HTML 컴포넌트를 삽입하여 소리 재생 명령 실행
                    components.v1.html(JS_SHUTTER_PLAY, height=0)
            except queue.Empty:
                pass
            except Exception:
                pass
