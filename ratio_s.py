...
def main():
    ...
    # ⭐ 셔터 소리 재생을 위한 HTML/JavaScript 정의
    SHUTTER_HTML = """
    <audio id="shutter_sound" src="shutter.mp3" preload="auto"></audio> 
    <script>
    document.getElementById('shutter_sound').currentTime = 0; 
    document.getElementById('shutter_sound').play().catch(function(error) {
    ...
    if webrtc_ctx.state.playing and webrtc_ctx.video_processor:
        ...
        while True:
            try:
                result_img_rgb = processor.result_queue.get(timeout=0.01)  # 타임아웃 조정
            except queue.Empty:
                result_img_rgb = None
